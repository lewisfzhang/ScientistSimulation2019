...
class Collect:
    ...
    def collect_data(self):
        ...
        sci_overall_returns_tp = [sci.overall_returns_tp for sci in self.m.scientist_list]  # Overall returns by time period, DataFrame
        ...
        sci_returns_tot_df = array2d_to_df(sci_returns_tot, row_name='sci', col_name='idea', file_name='sci_returns_tot')
        sci_overall_returns_tp_df = array2d_to_df(sci_overall_returns_tp, row_name='sci', col_name='tp', file_name='sci_ideas_k_paid_tot')
        sci_ideas_k_paid_tot_df = array2d_to_df(sci_ideas_k_paid_tot, row_name='sci', col_name='idea', file_name='sci_ideas_k_paid_tot')
        arrays_to_html([sci_returns_tot_cum], ['sci_returns_tot_cum'], 'scientist')  # if more than one element, create a list of variables
        ...
        idea_sci_impact = [idea.get_returns(i.idea_mean, i.idea_sds, i.idea_max, 0, i.total_effort) for i in self.m.idea_list]  # Total scientific impact, based on point on the idea curve, Array
        idea_inflect_tp = [sum_point_array(i.effort_by_tp, i.idea_mean) for i in self.m.idea_list]  # Time period where lambda was reached, Array --> at mean?

        idea_effort_by_tp_df = array2d_to_df(idea_effort_by_tp, row_name='idea', col_name='tp', file_name='idea_effort_by_tp')
        idea_num_k_by_tp_df = array2d_to_df(idea_num_k_by_tp, row_name='idea', col_name='tp', file_name='idea_num_k_by_tp')
        arrays_to_html([idea_total_effort, idea_num_k, idea_sci_impact, idea_inflect_tp],
